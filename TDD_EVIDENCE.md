...
2. **スクロール位置に応じてナビが自動ハイライト**
   - IntersectionObserver で現在位置を検出
   - アクティブなセクションボタンを自動更新
...
| 6 | モバイル表示で対応 | SectionNav.test.tsx | Unit | ✅ PASS | npm test -- src/components/SectionNav.test.tsx |
| 7 | セクションナビゲーション表示される | App.test.tsx | Integration | ✅ PASS | npm test -- src/App.test.tsx |
| 8 | 5つのセクションボタン表示される | App.test.tsx | Integration | ✅ PASS | npm test -- src/App.test.tsx |
| 9 | 各セクション ID 属性を持つ | App.test.tsx | Integration | ✅ PASS | npm test -- src/App.test.tsx |
| 10 | トップバーとセクションナビが共存 | App.test.tsx | Integration | ✅ PASS | npm test -- src/App.test.tsx |
...
⏳ **IntersectionObserver ハイライト更新**
- 現在位置に基づく自動ハイライト切り替え
- 実装は予定していたが、issue要件では「余力があれば」のため、基本機能完了で十分
...
- **App.tsx**: 既存5 + 新規4 = 9テストすべてパス
- **全プロジェクト**: 222/222 テストパス
