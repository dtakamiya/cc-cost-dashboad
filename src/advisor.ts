...
export const UNEXPLAINED_CACHE_BUST_COST_THRESHOLD = 0.01; // 原因不明のキャッシュ再作成コスト(USD)がこの値超で提案
...
const MCP_UNUSED_SERVER_NAME_LIMIT = 5; // 未使用MCPサーバーをdetailに列挙する上限件数
...
export function buildRecommendations(s: Summary, billingMode: BillingMode = "api"): AdvisorResult {
  ...
  // 5e. 原因不明のキャッシュ再作成（medium, 定量）
  // モデル切替・アイドルギャップのどちらにも該当しないキャッシュ再作成は、
  // セッション途中のMCP/設定変更やツール利用状況の変化が原因である可能性が高い。
  const ub = s.unexplainedCacheBust;
  if (billingMode !== "subscription" && ub && ub.reCreateCost > UNEXPLAINED_CACHE_BUST_COST_THRESHOLD) {
    items.push({
      id: "unexplained-cache-bust",
      priority: "medium",
      title: "原因不明のキャッシュ再作成（cache bust）",
      shortTitle: `原因不明のキャッシュ再作成（${ub.bustCount}回）`,
      detail: `モデル切替・アイドルギャップのいずれにも該当しない ${ub.bustCount} 回のキャッシュ再作成で、約 ${ub.reCreateTokens.toLocaleString()} トークンが再課金された（約 ${ub.reCreateCost.toFixed(2)} USD）。セッション途中のMCP/設定変更やツール構成の変化が原因の可能性がある。`,
      action: "セッション途中でのMCPサーバー有効化・設定変更・システムプロンプト変更を避け、同一タスク内では構成を固定する。",
      estMonthlySavings: ub.reCreateCost * monthlyFactor,
    });
  }

  // 5f. MCPサーバオーバーヘッド過大 → 未使用MCPの無効化検討（medium, 定量）
  ...
  {
    const unusedServers = s.overhead.mcpServers.filter((m) => m.callCount === 0);
